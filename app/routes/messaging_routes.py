...
def can_access_conversation(user, role, conversation):
    ...
    if conversation.conversation_type == 'department_private':
...
def recipients_for_conversation(conversation, sender, sender_role):
    ...
    if conversation.conversation_type in ('department', 'department_private') and conversation.department:
...
def serialize_conversation_summary(conversation, user, role, audit=False):
    ...
    return {
        ...
        'is_restricted': conversation.conversation_type in ('department_private', 'private_channel'),
        'audit_only': audit,
...
def create_channel():
    ...
    if conversation_type not in {'channel', 'private_channel', 'department', 'department_private'}:
        return jsonify({"error": "Invalid conversation type."}), 400

    if not name:
        return jsonify({"error": "Channel name is required."}), 400

    if conversation_type in ('department', 'department_private'):
    ...
    return jsonify({
        ...
        "is_restricted": channel.conversation_type in ('department_private', 'private_channel')
    }), 201
...
def rename_channel(channel_id):
    ...
    if conversation.conversation_type not in ('channel', 'department', 'department_private', 'private_channel'):
